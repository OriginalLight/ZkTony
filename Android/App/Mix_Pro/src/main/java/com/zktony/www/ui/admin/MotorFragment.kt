            ...
            viewLifecycleOwner.lifecycle.repeatOnLifecycle(Lifecycle.State.STARTED) {
                launch {
                    viewModel.uiState.collect {
                        ...
                        if (it.motor != null) {
                            binding.apply {
                                tvTitle.text = it.motor.text
                                speed.setEqualText(it.motor.speed.toString())
                                acceleration.setEqualText(it.motor.acc.toString())
                                deceleration.setEqualText(it.motor.dec.toString())
                            }
                        }
                        binding.update.isEnabled = it.motor != null && it.motor.speed > 0 && it.motor.acc > 0 && it.motor.dec > 0
                    }
                }
            }
        }
